...
更新时保持浅克隆，执行 pull 时同样指定 `--depth 10`，避免拉取过多历史提交：

```shell
$ git pull --depth 10 || git pull
```
...
* [iptv-unicast.m3u](https://github.com/zzzz0317/beijing-unicom-iptv-playlist/raw/refs/heads/main/iptv-unicast.m3u): 转换为单播地址的播放列表，需要使本地网络能解析 `iptv.local` 域名到您可访问的 IPTV 代理程序实例（如 `udpxy`、`rtp2httpd` 等），您也可以将文件中的 `iptv.local` 替换为您可访问的 IPTV 代理程序实例地址
